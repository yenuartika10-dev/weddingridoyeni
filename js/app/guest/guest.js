...
export const guest = (() => {
    ...
    const booting = async () => {
        ...
        const presenceForm = document.getElementById('form-presence');

        if (presenceForm && information.has('presence')) {
            presenceForm.value = information.get('presence') ? '1' : '2';
        }
    ...
    const pageLoaded = () => {
        ...
        const hasCommentSection = Boolean(document.getElementById('comments') && document.getElementById('pagination'));
        ...
        if (!token || token.length <= 0 || !hasCommentSection) {
